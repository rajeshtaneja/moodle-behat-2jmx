...
class testplan_writer {
    ...
    public static function start_testplan($size) {
        ...
        // If this site is installed by moodle-behat-generator, then we set config performancesitedata = sitesize,version
        if ($config = get_config('core', 'performancesitedata')) {
            list($dataplansize, $dataplanversion) = explode(',', $config);
        } else {
            $dataplansize = 'unknown';
            $dataplanversion = 'unknown';
        }
